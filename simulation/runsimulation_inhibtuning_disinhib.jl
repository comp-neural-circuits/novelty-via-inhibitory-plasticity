...
function runsimulation_inhibtuning_disinhib(ifiSTDP,ifwadapt,stimparams,stimulus::Array{Float64,2},
	...
	"""	Runs a new simulation of a plastic E-I spiking neural network model where both E and I
		...
		tauw_adapt = 150 #adaptation timescale ms
		...
		rix = 2.5#25 #external input rate to i (khz)
		...
		#Nass = size(assemblymembers,1) #number of assemblies
		...
		Nass = Nassemblies#maximum(Nimg)*maximum(Nseq)
		nm = zeros(Int32,Nass)
		inhibnm = zeros(Int32,Nass)
		...
		avgXassembly = zeros(Float64,Nass,Nass)
		avgInhibXassembly = zeros(Float64,Nass,Nass)
		avgItoassembly = zeros(Float64,Nass)
		avgnonmemstoassembly = zeros(Float64,Nass)
		avgassemblytononmems = zeros(Float64,Nass)
		avgassemblytonovelty = zeros(Float64,Nass)
		avgnoveltytoassembly = zeros(Float64,Nass)
	...
	@time	for tt = 1:Nsteps
				...
				for ss = 1:size(stimulus)[1]
					...
					if (tprev<stimulus[ss,2]) && (t>=stimulus[ss,2])  #just entered stimulation period
						...
						if ass > Nimg*Nseq && (t>stimulus[lenpretrain,2]) #&& ass <= Nimg*Nseq + Nseq # if novelty assembly
							# 	#idxnovelty .= shuffle(collect(1:Ne))[1:Nmembersmax]
							idxnovelty = assemblymembers[ass,assemblymembers[ass,:] .!= -1] # average number of neurons per assembly
							novelstore += 1
							#println("Ass $(ass) novelstore $(novelstore)")
						...
						# --------------- specific to oddball paradigm ----------
						# reset to assembly A if not AA instead of AB
						if round(Int,stimulus[ss,1]) > 1 && ifAAnotAB
							ass = 1
						end
					...
					if (tprev<stimulus[ss,3]) && (t>=stimulus[ss,3]) #just exited stimulation period
						...
						# --------------- specific to oddball paradigm ----------
						# reset to assembly A if not AA instead of AB
						if round(Int,stimulus[ss,1]) > 1 && ifAAnotAB # ensure only ass is set to 1 when the flag is set
							println("assembly set to 1")
							ass = 1
						end
