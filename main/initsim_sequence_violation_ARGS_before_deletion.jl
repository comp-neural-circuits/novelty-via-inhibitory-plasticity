...
# stimulus parameters
Nimg = parse(Int64, ARGS[1])		# number of images per sequence
...
adjustfactor = parse(Int64, ARGS[8])/10 # ignore input here
#adjustfactor = 1
...
if usetrained
	filesavename = "incrINHIB_$(inhibfactor)__ifinhibhetero_$(inhibhetero)_Traineddur$(T)msNblocks$(Nblocks)Ntrain$(Ntrain)lenstim$(lenstim)lenpause$(lenpause)Nreps$(Nreps)strength$(strength)wadapt$(ifwadapt)iSTDP$(ifiSTDP)RateAdjust$(adjustfactor)Time"
else
	filesavename = "incrINHIB_$(inhibfactor)__ifinhibhetero_$(inhibhetero)_dur$(T)msNblocks$(Nblocks)Ntrain$(Ntrain)lenstim$(lenstim)lenpause$(lenpause)Nreps$(Nreps)strength$(strength)wadapt$(ifwadapt)iSTDP$(ifiSTDP)RateAdjust$(adjustfactor)Time"
end
...
assemblymembers, spiket, storagedec, storagetimes,savefile, lenpretrain, inhibassemblies,
 adjustfactor = adjustfactor, adjustfactorinhib = adjustfactorinhib,
   inhibfactor = inhibfactor, inhibhetero = inhibhetero, T =1)
# run main simulation
@time totalspikes = runsimulation_inhibtuning(ifiSTDP,ifwadapt,stimparams,stimulus,
 ...
 inhibassemblies, adjustfactor = adjustfactor, adjustfactorinhib = adjustfactorinhib,
  inhibfactor = inhibfactor, inhibhetero = inhibhetero, T = T)
